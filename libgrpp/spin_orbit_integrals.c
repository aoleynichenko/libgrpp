 ...
 * However, the formula on page 776 of Pitzer & Winter is not reproduced in the code exactly.
 */
void libgrpp_spin_orbit_integrals(libgrpp_shell_t *shell_A, libgrpp_shell_t *shell_B,
                                  ...
                                  double *so_x_matrix, double *so_y_matrix, double *so_z_matrix)
{
    ...
    int L = potential->L;
    int L_A = shell_A->cart_list[0] + shell_A->cart_list[1] + shell_A->cart_list[2];
    int L_B = shell_B->cart_list[0] + shell_B->cart_list[1] + shell_B->cart_list[2];
    ...
    for (int lambda = 0; lambda <= lmax; lambda++) {
        evaluate_real_spherical_harmonics_array(lambda, kA_vec, rsh_values_kA[lambda]);
        evaluate_real_spherical_harmonics_array(lambda, kB_vec, rsh_values_kB[lambda]);
    }
                        ...
                        for (int d = 0; d <= n_B; d++) {
                            ...
                            for (int e = 0; e <= l_B; e++) {
                                ...
                                for (int f = 0; f <= m_B; f++) {
                                    ...
                                    int N = a + b + c + d + e + f;
                                    double factor = C_nA_a * C_lA_b * C_mA_c * C_nB_d * C_lB_e * C_mB_f *
                                                    pow_CA_x * pow_CA_y * pow_CA_z * pow_CB_x * pow_CB_y * pow_CB_z;
                                    ...
                                    for (int lambda_1 = lambda1_lower; lambda_1 <= lambda1_upper; lambda_1++) {
                                        if ((L + a + b + c - lambda_1) % 2 != 0) {
                                            continue;
                                        }

                                        for (int lambda_2 = lambda2_lower; lambda_2 <= lambda2_upper; lambda_2++) {
                                            if ((L + d + e + f - lambda_2) % 2 != 0) {
                                                continue;
                                            }

                                            double QN = get_radial_type2_integral(radial_table, lambda_1, lambda_2, N);
                                            if (fabs(QN) < LIBGRPP_ZERO_THRESH) {
                                                continue;
                                            }

                                            double sum_angular_x, sum_angular_y, sum_angular_z;
                                            type3_angular_sum(L, Lx_matrix, Ly_matrix, Lz_matrix,
                                                              lambda_1, a, b, c, rsh_values_kA[lambda_1],
                                                              lambda_2, d, e, f, rsh_values_kB[lambda_2],
                                                              &sum_angular_x, &sum_angular_y, &sum_angular_z);
    ...
    *sum_angular_z = 0.0;
    ...
    for (int m1 = -L; m1 <= L; m1++) {
        for (int m2 = -L; m2 <= L; m2++) {
            ...
            if (fabs(lx) < LIBGRPP_ZERO_THRESH &&
                fabs(ly) < LIBGRPP_ZERO_THRESH &&
                fabs(lz) < LIBGRPP_ZERO_THRESH) {
                continue;
            }

            double omega_1 = angular_type2_integral(lambda_1, L, m1, a, b, c, rsh_values_kA);
            if (fabs(omega_1) < LIBGRPP_ZERO_THRESH) {
                continue;
            }

            double omega_2 = angular_type2_integral(lambda_2, L, m2, d, e, f, rsh_values_kB);
